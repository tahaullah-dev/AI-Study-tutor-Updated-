...
if (summarizeBtn) {
  summarizeBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      const cleanSummary = (resp.summary || '').replace(/\*\*(.*?)\*\*/g, '$1');
      if (summaryDisplay) summaryDisplay.textContent = cleanSummary;
      showSummary(true);
    } catch (err) {
...
if (copySummary) {
  copySummary.addEventListener('click', async () => {
    const text = summaryDisplay ? summaryDisplay.textContent.trim() : '';
    if (!text) return;
    try {
